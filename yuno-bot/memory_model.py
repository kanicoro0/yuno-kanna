...
MEMORY_RECORD_SCHEMA_VERSION = 4
...
MEMORY_RECORD_TYPE_LABELS = {
    "memory": "覚えていること",
    "interaction_preference": "話し方・扱い方",
}


def memory_record_type_label(record_type):
    record_type = normalize_memory_record_type(record_type)
    return MEMORY_RECORD_TYPE_LABELS.get(record_type, "覚えていること")


def memory_record_type_default_category(record_type):
    record_type = normalize_memory_record_type(record_type)
...
def normalize_memory_record_type(value, *, fallback_category=None):
    requested = str(value or "").strip()
    if requested in MEMORY_RECORD_TYPES:
        return requested
    if requested in MEMORY_RECORD_TYPE_LABELS.values():
        return next(
            record_type
            for record_type, label in MEMORY_RECORD_TYPE_LABELS.items()
            if label == requested
        )
...
def _operation_record_type(operation):
    if not isinstance(operation, dict):
        return "memory"
    return normalize_memory_record_type(
        operation.get("record_type"),
        fallback_category=operation.get("category"),
    )
...
def normalize_auto_memory_operations(operations):
    ...
    for index, operation in enumerate(operations[:10]):
        ...
        record_type = normalize_memory_record_type(
            operation.get("record_type"),
            fallback_category=operation.get("category"),
        )
        if record_type not in MEMORY_RECORD_TYPES:
            errors.append(f"{label}: record_type が使用できない")
            continue
...
def format_memory_records_for_display(
    ...
    *,
    ...
    if collection in {"memories", "keywords", "interaction_preferences"}:
        legacy_collection = collection
    elif collection in {"all", "records", ""}:
        legacy_collection = None
        collection = "all" if collection != "records" else "records"
    else:
        return ["collection は省略するか records / all を指定してね"]
    ...
    lines = [
        "🧾 記憶records",
        f"status: {status} / scope: {collection} / page: {page}/{total_pages}",
        (
            "summary: "
...
def memory_record_is_deleted(user_id, *, collection="records", record_id):
    collection = str(collection or "records").strip().lower()
    record_id = str(record_id or "").strip()
    if collection in {"all", ""}:
        collection = "records"
...
def _v3_find_record_by_id(entry, record_id, collection=None):
    ...
    if collection in (None, "", "records", "all"):
        records = entry.get("records", {})
        if isinstance(records, dict):
...
def _v3_find_record_any(
    ...
    *,
    ...
    expected_type = normalize_memory_record_type(record_type, fallback_category=category)
...
async def restore_memory_record(user_id, *, record_id, collection="records"):
    ...
    async with memory_lock:
        ...
        append_memory_change(
            entry,
            source="manual",
            summary=f"{memory_record_type_label(record_type)}を復元",
            changes=[change],
        )
...
async def apply_v3_memory_operations(user_id, operations, *, summary, source):
    ...
    async with memory_lock:
        ...
        for operation in operations:
            ...
            record_type = _operation_record_type(operation)
    ...
    for index, operation in enumerate(raw_operations[:20]):
        ...
        if operation_type == "clear_category":
            ...
            if not category and operation.get("record_type"):
                category = memory_record_type_default_category(
                    operation.get("record_type")
                )
            if not category:
                errors.append(f"{label}: record_type が使用できない")
        ...
        if operation_type == "delete_matching_items":
            ...
            if not query or len(query) > 100:
                ...
                continue
            category_filter = (
                canonicalize_memory_category(operation.get("category"))
                if operation.get("category")
                else None
            )
            record_type_filter = (
                normalize_memory_record_type(operation.get("record_type"))
                if operation.get("record_type")
                else None
            )
            targets = []
            items = entry.get("items", {}) if isinstance(entry, dict) else {}
            for category in ordered_memory_categories(entry):
                ...
                if (
                    record_type_filter
                    and memory_category_display_group(category)
                    != memory_record_type_label(record_type_filter)
                ):
                    continue
...
def describe_memory_operation(operation):
    ...
    record_label = memory_record_type_label(_operation_record_type(operation))
    if operation_type == "add_item":
        return f"{record_label}に追加: {operation.get('item')}"
    if operation_type == "delete_item":
        return f"{record_label}から削除: {operation.get('item')}"
    if operation_type == "rewrite_item":
        return (
            f"{record_label}を書き換え: "
            ...
            if isinstance(targets, list) and targets:
                for target in targets:
                    _, _, record = _v3_find_record_any(
                        ...
                        record_type=_operation_record_type(target),
                    )
                    if record is None:
                        return False
            else:
...
def _v3_undo_operation(entry, operation):
    ...
    if operation_type == "rewrite_item":
        ...
        record["type"] = _operation_record_type(operation)
        record["updated_at"] = datetime.now().isoformat(timespec="seconds")
        return True
