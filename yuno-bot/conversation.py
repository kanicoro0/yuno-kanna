...
async def on_message(message):
    ...
    if "ゆの" in lowered or "唯乃" in lowered or "yuno" in lowered:
        try:
            ...
            prompt = ("""次の会話ログの最後の発言を判定してください。
返信本文は書かず、必ず1行だけで答えてください。

最後の発言は「ゆの」への質問・呼びかけ・相談ですか？
単に「ゆの」という語が含まれるだけなら「いいえ」。
...
・明確に操作できる変更がない場合は空配列にする
・記憶するのは、本人が明示した安定情報と、ゆのへの接し方の希望だけ
・一時的な気分、推測、他人の個人情報、センシティブな情報は記憶しない
・memory は本人について覚える内容、interaction_preference は話し方・扱い方への希望に使う
・削除や書き換えは、現在の記憶から対象を完全に特定できる場合だけ行う
・曖昧な削除や整理依頼では操作を書かず、replyで短く確認する
・item / old_item / new_item は1件だけ書く。複数ある場合は operation を分ける
・現在の記憶と同じ内容は出力しない
"""
