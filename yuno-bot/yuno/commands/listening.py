...
def create_listening_group(service: ListeningChannelService) -> app_commands.Group:
    ...
    async def listening_add(
        ...
        text = (
            "このチャンネルを聞くようにした"
            if result.changed
            else "このチャンネルはもう聞いている"
        )
        await _reply(interaction, text)

    @group.command(name="remove", description="聞く場所を解除")
    async def listening_remove(
        ...
        if result.reason == "env_protected":
            ...
            text = "このチャンネルは聞いていないよ"
        await _reply(interaction, text)

    @group.command(name="clear", description="このサーバーの聞く場所を解除")
    async def listening_clear(interaction: discord.Interaction) -> None:
        ...
        text = f"聞く場所を{count}件外した" if count else "外せる場所はなかった"
        await _reply(interaction, text)
...
def _channel_exists_in_guild(
    ...
    if guild is None:
        return False
    try:
        channel_id = int(item.discord_channel_id)
    except ValueError:
        return False
