...
class RuntimeSettings:
    ...
    def _normalize(payload: Any) -> Dict[str, Any]:
        ...
        notebook_view = payload.get("notebook_view") if isinstance(payload.get("notebook_view"), dict) else {}
        ...
        view_users = notebook_view.get("users") if isinstance(notebook_view.get("users"), dict) else {}
        return {
            "schema_version": 1,
            "sleep": {
    ...
    async def notebook_view(self, user_id: int) -> str:
        data = await self.snapshot()
        return data["notebook_view"]["users"].get(str(user_id), "normal")

    async def set_notebook_view(self, user_id: int, mode: str) -> None:
        if mode not in {"normal", "debug"}:
            raise ValueError("invalid notebook view mode")
        async with self._lock:
            data = await self._data_locked()
            data["notebook_view"]["users"][str(user_id)] = mode
            await self._save_locked()
