...
- 操作を頼まれたのに状態が変わらなかった時は、「変えたと言わない」旨のメモが渡ります。
  頼まれた証拠は発話の言葉だけでなく、CareReader の提案が却下されたことも含みます
- 操作は読み取れたが、どの印のことか決められない時は、状態を変えず、
  Speaker を通さない短い定型の聞き返しを確実に返します
- 元に戻す操作は、まだ会話からはできません。記憶を明示した戻す依頼には、
  戻したと言わず、一覧から戻せることだけを伝えます。
  ただし「うん」のような続きの確認発言には検知できる印がなく、
  その次のターンで戻したと言わない線は Speaker への指示だけに支えられています。
  この多ターン経路の機械的な保証は未対応です
- Speaker は、知らされていないのに覚えた・忘れた・閉じた・戻した・変えたと言い切りません

聞き返しのあとの一往復は、保留中の操作としてつながります。

- 聞き返した直後だけ、その stream と話者に対する保留中の操作を短時間だけ覚えます
- 次の発言が対象を答えていれば、最初に頼まれた操作として扱います
- まだ決められなければ、もう一度聞き返しに戻ります
- 対象を伴う新しい明示操作が来た時は、古い保留を回答として使わず破棄し、
  新しい依頼として扱います。「やめて」「もういい」のような弱い言い方だけでは破棄しません
- 保留は 1 stream に 1 つ、数分で消え、DB には持ちません。消えたら、もう一度頼めば済みます
